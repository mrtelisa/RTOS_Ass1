...
main()
{
	...
	iret[4] = pthread_create( &(thread_id[4]), &(attributes[4]), task_per, NULL);
...
void *task_per( void *ptr)
{
	...
	int i=0;
  	for (i=0; i < 25; i++)
		...
		// Please be careful: the index 0 in all structures now refers to the Polling Server,
		// whereas the fourth periodic task is now assigned the index 4
